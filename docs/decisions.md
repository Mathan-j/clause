...
- fastapi + uvicorn — the HTTP surface for the answering pipeline. Chosen over a bare WSGI app because the project's own brief names FastAPI, and over adding a template engine because the UI is a single static page that talks to two JSON endpoints. The API deliberately adds no behaviour: it serialises `answer_one` and the citation resolver, so a question asked over HTTP scores exactly the way the evaluation harness scores it.
