...
There is a [two-minute narrated demo](docs/video/clause-demo.mp4) in this repo.
...
Full reports: [`reports/eval.md`](reports/eval.md) for retrieval, regenerated by
`make eval` and gated in CI, and [`reports/answers.md`](reports/answers.md) for
answering and refusal. Every figure below comes from those committed files.

![the evaluation, in the UI](docs/screenshots/04-evidence-measured.png)
...
### The citation contract, measured

Of the drafts the model generated, **8.3% were rejected** by the citation contract —
they cited something that did not resolve, or asserted a fact with no citation, and
were discarded rather than returned. That is the contract working: a returned answer
carrying an unresolvable citation is impossible by construction, because `enforce`
raises before one is built. The figure measures the generator, not the enforcement.

Support signal averaged **0.822** across 115 factual sentences, with 90 of them
distinct — the 3B model repeats itself about a fifth of the time.
...
bucket by ~6 points. The support signal is term overlap, which is a proxy for faithfulness and not
entailment: it catches a sentence citing a passage it shares no vocabulary with, and
misses a fluent paraphrase that reverses the meaning.
